...
const LearnAboutCredits = () => {
  ...
  const renderContent = () => {
    const spentTransactions = creditTransactions.filter(
      (item) => item.transaction_type === TRANSACTION_POST || item.transaction_type === TRANSACTION_TRANSFER || item.transaction_type === TRANSACTION_SUBSCRIBE 
    );
  
    const receivedTransactions = creditTransactions.filter(
      (item) => item.transaction_type === TRANSACTION_RELOAD || item.transaction_type === TRANSACTION_RECEIVED || item.transaction_type === TRANSACTION_SUBSCRIBER
    );
            ...
            {spentTransactions.map((item, index) => (
              <Card key={index} style={{ marginVertical: 10 }}>
                <Card.Content style={styles.cardContainer}>
                  ...
                  <Text style={[styles.amountText, { color: (item.transaction_type === TRANSACTION_POST|| item.transaction_type === TRANSACTION_TRANSFER|| item.transaction_type === TRANSACTION_SUBSCRIBE) ? '#FF0000' : 'black' }]}> 
                    {item.amount} credits
                  </Text>
                </Card.Content>
              </Card>
