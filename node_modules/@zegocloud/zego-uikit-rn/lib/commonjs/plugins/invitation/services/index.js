...
var _signal_plugin = _interopRequireDefault(require("../../../signal_plugin"));
...
const ZegoUIKitSignalingPluginImpl = {
  ...
  init: (appID, appSign) => {
    if (_ZegoUIKitCorePlugin.default.getPlugin(_defines.ZegoUIKitPluginType.signaling)) {
      ZegoUIKitSignalingPlugin = _signal_plugin.default;
    }
    if (!ZegoUIKitSignalingPlugin) {
      (0, _logger.zlogerror)(`[Plugins][invitation]Signaling plugin install error.`);
      return;
  ...
  joinRoom(roomID) {
    ...
    return new Promise((resolve, reject) => {
      ZegoUIKitSignalingPlugin.getInstance().invoke('joinRoom', {
        ...
        try {
          (0, _logger.zloginfo)('[Plugins][invitation]Join room successfully.', roomID);
  ...
  queryUsersInRoomAttributes: function (nextFlag) {
    ...
    return new Promise((resolve, reject) => {
      _queryUsersInRoomAttributesFully(nextFlag, count).then(fullAttributes => {
        ...
        resolve({
          ...new _defines2.ZegoInvitationImplResult('', ''),
  ...
  endRoomPropertiesBatchOperation() {
    ...
    return new Promise((resolve, reject) => {
      ZegoUIKitSignalingPlugin.getInstance().invoke('endRoomPropertiesBatchOperation').then(async () => {
        try {
          (0, _logger.zloginfo)('[Plugins][invitation]End room properties batch operation successfully.');
          // query the room attributes
          // await this.queryRoomProperties();
          // zloginfo(
          //   `[Plugins][invitation]Auto query the room attributes successfully.`,
  ...
  onInRoomCommandMessageReceived(callbackID, callback) {
    ...
    ZegoUIKitSignalingPlugin.getInstance().registerPluginEventHandler('onInRoomCommandMessageReceived', callbackID, callback);
  }
};
var _default = exports.default = ZegoUIKitSignalingPluginImpl;
