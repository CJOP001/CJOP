...
var _defines2 = _interopRequireDefault(require("./defines"));
...
class ZegoSignalingPluginCore {
  constructor() {
    ...
    // <zim call id, user id>
    _defineProperty(this, "_connectionState", _defines.ZIMConnectionState.Disconnected);
  ...
  _registerEngineCallback() {
    ...
    _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().on('connectionStateChanged', (zim, _ref) => {
      let {
        state,
        event,
        extendedData
      } = _ref;
      (0, _logger.zloginfo)(`[Core]Connection state changed, state:${state}, event:${event}, extended data:${extendedData}`);
      this._connectionState = state;
      this._notifyConnectionStateChanged({
        state
      });
      if (this._connectionState === _defines.ZIMConnectionState.Disconnected) {
        (0, _logger.zlogwarning)('[Core]Disconnected, auto logout.');
        // this.logout();
      }
    });
    // Callback of the call invitation received by the invitee.
    _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().on('callInvitationReceived', (zim, _ref2) => {
      let {
        callID,
        inviter,
        timeout,
      ...
      (0, _logger.zloginfo)('[Core][callInvitationReceived callback]', callID, inviter, timeout, extendedData);
    ...
    _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().on('callInvitationCancelled', (zim, _ref3) => {
      let {
        callID,
        inviter,
        extendedData
      } = _ref3;
      (0, _logger.zloginfo)('[Core][callInvitationCancelled callback]', callID, inviter, extendedData);
    ...
    _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().on('callInvitationAccepted', (zim, _ref4) => {
      let {
        callID,
        invitee,
        extendedData
      } = _ref4;
      (0, _logger.zloginfo)('[Core][callInvitationAccepted callback]', callID, invitee, extendedData);
      const notifyData = {
        callID,
        invitee: {
          id: invitee,
    ...
    _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().on('callInvitationRejected', (zim, _ref5) => {
      let {
        callID,
        invitee,
        extendedData
      } = _ref5;
      (0, _logger.zloginfo)('[Core][callInvitationRejected callback]', callID, invitee, extendedData);
      const notifyData = {
        callID,
        invitee: {
          id: invitee,
    ...
    _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().on('callInvitationTimeout', (zim, _ref6) => {
      let {
        callID
      } = _ref6;
      (0, _logger.zloginfo)('[Core][callInvitationTimeout callback]', callID);
      const notifyData = {
        callID,
        inviter: {
    ...
    _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().on('callInviteesAnsweredTimeout', (zim, _ref7) => {
      let {
        callID,
        invitees
      } = _ref7;
      (0, _logger.zloginfo)('[Core][callInviteesAnsweredTimeout callback]', callID, invitees);
      const notifyData = {
        callID,
  ...
  _unregisterEngineCallback() {
    ...
    _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().off('callInvitationCancelled');
    _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().off('callInvitationAccepted');
    _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().off('callInvitationRejected');
    _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().off('callInvitationTimeout');
    _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().off('callInviteesAnsweredTimeout');
  }
  ...
  _resetDataForLogout() {
    ...
    this._connectionState = _defines.ZIMConnectionState.Disconnected;
  }
  ...
  getCallIDByUserID(userID) {
    ...
    Array.from(this._callIDUsers.keys()).forEach(key => {
      ...
      if (userID === value) {
        callID = key;
        (0, _logger.zloginfo)('[Core]getCallIDByUserID', userID, this._callIDUsers, callID);
      }
    });
    return callID;
  }
  ...
  create(appConfig) {
    if (!_ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance()) {
      ...
      (0, _logger.zlogwarning)('[Core]Zim has created.');
      this._createHandle();
    }
  }
  login(userInfo) {
      ...
      if (!this._isLogin) {
        _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().login(userInfo, token).then(() => {
          ...
          if (error.code == 6000111) {
            (0, _logger.zloginfo)('[Core]Login already success.', error);
            resolve();
          } else {
            reject(error);
          }
  ...
  invite(invitees, config) {
    return new Promise((resolve, reject) => {
      ...
      console.warn(config);
      _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().callInvite(invitees, config).then(_ref8 => {
        let {
          callID,
          timeout,
          errorInvitees
        ...
        if (!errorInvitees || !errorInvitees.length) {
          ...
          errorInvitees.forEach(errorInvitee => {
            const desc = errorInvitee.state === _defines.ZIMCallUserState.Offline ? 'offine' : errorInvitee.state === _defines.ZIMCallUserState.Inviting ? 'inviting' : '';
  ...
  cancel(invitees, callID, config) {
    return new Promise((resolve, reject) => {
      _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().callCancel(invitees, callID, config).then(_ref9 => {
        let {
          callID: resCallID,
          errorInvitees
        } = _ref9;
  ...
  accept(callID, config) {
    return new Promise((resolve, reject) => {
      _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().callAccept(callID, config).then(_ref10 => {
        let {
          callID: resCallID
        } = _ref10;
        (0, _logger.zloginfo)(`[Core]Accept invitation done, call id: ${callID}`);
        resolve(new _defines2.default());
      }).catch(error => {
        reject(error);
      });
    });
  ...
  reject(callID, config) {
    return new Promise((resolve, reject) => {
      _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().callReject(callID, config).then(_ref11 => {
        let {
          callID: resCallID
        } = _ref11;
  ...
  queryCallList(config) {
    return new Promise((resolve, reject) => {
      _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().queryCallInvitationList(config).then(_ref12 => {
        let {
          callList,
          nextFlag: newFlag
        } = _ref12;
  ...
  onCallInvitationCancelled(callbackID, callback) {
    if (typeof callback !== 'function') {
      if (callbackID in this._onCallInvitationCancelledCallbackMap) {
        (0, _logger.zloginfo)('[Core][onCallInvitationCancelled] Remove callback for: [', callbackID, '] because callback is not a valid function!');
