...
import { StyleSheet, View, SafeAreaView, Platform, KeyboardAvoidingView, TouchableOpacity } from 'react-native';
import ZegoUIKit, { ZegoRoomPropertyUpdateType, ZegoInRoomMessageInput, ZegoInRoomMessageView, ZegoAudioVideoContainer, ZegoLayoutMode, ZegoSwitchCameraButton, ZegoUIKitPluginType } from '@zegocloud/zego-uikit-rn';
...
import { HOST_DEFAULT_CONFIG, AUDIENCE_DEFAULT_CONFIG, ZegoLiveStreamingRole, ZegoLiveStatus, ZegoTranslationText, ZegoInvitationType, ZegoCoHostConnectState, ZegoToastType } from "./services/defines";
...
export { ZegoMenuBarButtonName, HOST_DEFAULT_CONFIG, AUDIENCE_DEFAULT_CONFIG, ZegoUIKitPrebuiltLiveStreamingFloatingMinimizedView };
...
function ZegoUIKitPrebuiltLiveStreaming(props, ref) {
  ...
  const {
    ...
    audienceButtons = plugins && plugins.length && plugins.find(plugin => plugin.ZIMConnectionState) ? [ZegoMenuBarButtonName.coHostControlButton] : [],
    hostExtendButtons = [],
    coHostExtendButtons = [],
  ...
  const [turnOnCameraWhenJoining, setTurnOnCameraWhenJoining] = useState(stateData.current.turnOnCameraWhenJoining !== undefined ? stateData.current.turnOnCameraWhenJoining : config.turnOnCameraWhenJoining);
  const [turnOnMicrophoneWhenJoining, setTurnOnMicrophoneWhenJoining] = useState(stateData.current.turnOnMicrophoneWhenJoining !== undefined ? stateData.current.turnOnMicrophoneWhenJoining : config.turnOnMicrophoneWhenJoining);
  const [useSpeakerWhenJoining, setUseSpeakerWhenJoining] = useState(stateData.current.useSpeakerWhenJoining !== undefined ? stateData.current.useSpeakerWhenJoining : config.useSpeakerWhenJoining);
  ...
  const [memberConnectStateMap, setMemberConnectStateMap] = useState(stateData.current.memberConnectStateMap || {}); // The connection status of the current member
  ...
  const registerPluginCallback = () => {
    if (ZegoUIKit.getPlugin(ZegoUIKitPluginType.signaling)) {
      ZegoUIKit.getSignalingPlugin().onInvitationReceived(callbackID, _ref => {
        let {
          callID,
          type,
          inviter,
        ...
        if (type === ZegoInvitationType.requestCoHost && userID === realTimeData.current.hostID) {
          ...
          setToastExtendedData({
            ...
            text: ZegoTranslationText.receivedRequestCoHostToast.replace('%0', inviter.id)
          });
      ...
      ZegoUIKit.getSignalingPlugin().onInvitationCanceled(callbackID, _ref2 => {
        let {
          inviter
        } = _ref2;
      ...
      ZegoUIKit.getSignalingPlugin().onInvitationTimeout(callbackID, _ref3 => {
        let {
          inviter
        } = _ref3;
        if (userID === realTimeData.current.hostID) {
      ...
      ZegoUIKit.getSignalingPlugin().onInvitationAccepted(callbackID, _ref4 => {
        let {
          invitee
        } = _ref4;
        if (userID === realTimeData.current.hostID) {
      ...
      ZegoUIKit.getSignalingPlugin().onInvitationRefused(callbackID, _ref5 => {
        let {
          invitee
        } = _ref5;
        if (userID === realTimeData.current.hostID) {
          ...
          setToastExtendedData({
            ...
            text: ZegoTranslationText.audienceRejectInvitationToast.replace('%0', invitee.id)
          });
      ...
      ZegoUIKit.getSignalingPlugin().onInRoomTextMessageReceived(callbackID, message => {
  ...
  const startLiveStreamingTimingTimer = useCallback(function () {
      ...
      if (ZegoUIKit.isRoomConnected()) {
        ...
        if (realTimeData.current.role === ZegoLiveStreamingRole.host && !realTimeData.current.hostID) {
          ZegoUIKit.updateRoomProperties({
            host: userID,
            live_status: `${ZegoLiveStatus.default}`
          });
  ...
  const showBottomBar = () => {
    // @ts-ignore
    return role === ZegoLiveStreamingRole.host && hostID && hostID === userID && liveStatus === ZegoLiveStatus.start || role === ZegoLiveStreamingRole.audience || role === ZegoLiveStreamingRole.coHost;
  };
  ...
  const onStartLiveStreaming = () => {
    ...
    ZegoUIKit.updateRoomProperties({
      host: hostID,
      live_status: `${ZegoLiveStatus.start}`
    }).then(() => {
  ...
  const tempHandle = async () => {
    ...
    } else if (role === ZegoLiveStreamingRole.coHost) {} else {}
  };
  ...
  return /*#__PURE__*/React.createElement(View, {
    ...
    audioVideoConfig: {
      ...
      cacheAudioVideoUserList: isMinimizeSwitch ? ZegoUIKit.getAllUsers().filter(user => user.userID && (user.isCameraOn || user.isMicrophoneOn)) : null
    },
    layout: {
    ...
    foregroundBuilder: foregroundBuilder || (_ref6 => {
      ...
      return /*#__PURE__*/React.createElement(ZegoAudioVideoForegroundView, {
        role: role,
        userInfo: userInfo,
        localUserID: userID,
  ...
  }, ZegoTranslationText.noHostOnline)))), /*#__PURE__*/React.createElement(View, {
    style: styles.topBarContainer
  }, /*#__PURE__*/React.createElement(View, {
    style: styles.left
  }, /*#__PURE__*/React.createElement(View, {
    style: [styles.hostInfo, role === ZegoLiveStreamingRole.host && hostID && liveStatus === ZegoLiveStatus.start || role !== ZegoLiveStreamingRole.host && hostID ? styles.show : null]
  }, /*#__PURE__*/React.createElement(View, {
    style: styles.avatar
  }, /*#__PURE__*/React.createElement(Text, {
    style: styles.nameLabel
  }, getShotName(getHostNameByID(hostID)))), /*#__PURE__*/React.createElement(Text, {
    style: styles.hostName
  }, getHostNameByID(hostID))), /*#__PURE__*/React.createElement(TouchableOpacity, {
    onPress: onNavBackPressed,
    style: [styles.navBackButton, role === ZegoLiveStreamingRole.host && hostID && liveStatus !== ZegoLiveStatus.start ? styles.show : null]
  ...
  // @ts-ignore
  role === ZegoLiveStreamingRole.host && liveStatus === ZegoLiveStatus.start || role !== ZegoLiveStreamingRole.host ? /*#__PURE__*/React.createElement(TouchableOpacity, {
  ...
  }, memberCount), requestCoHostCount ? /*#__PURE__*/React.createElement(View, {
    style: styles.memberRedDot
  }) : null)) : null,
  // @ts-ignore
  role === ZegoLiveStreamingRole.host && liveStatus === ZegoLiveStatus.start || role !== ZegoLiveStreamingRole.host ? /*#__PURE__*/React.createElement(ZegoTopBar, {
  ...
  // @ts-ignore
  role === ZegoLiveStreamingRole.host && hostID && hostID === userID && (liveStatus === ZegoLiveStatus.default || liveStatus === ZegoLiveStatus.default) ? /*#__PURE__*/React.createElement(View, {
    style: styles.switchCameraButton
  }, /*#__PURE__*/React.createElement(ZegoSwitchCameraButton, {
    iconFrontFacingCamera: require('./resources/icon_nav_flip.png'),
    iconBackFacingCamera: require('./resources/icon_nav_flip.png'),
    width: 36,
    height: 36
  })) : null)), showStartLiveButton() ? /*#__PURE__*/React.createElement(View, {
    style: styles.startLiveButtonCon
  }, startLiveButtonBuilder ? startLiveButtonBuilder(onStartLiveStreaming) : /*#__PURE__*/React.createElement(ZegoStartLiveButton, {
    onPressed: onStartLiveStreaming
  })) : null,
  // @ts-ignore
  isVisible && liveStatus === ZegoLiveStatus.start ? /*#__PURE__*/React.createElement(SafeAreaView, {
    style: styles.timingContainer
  }, /*#__PURE__*/React.createElement(Text, {
    style: styles.timing
  }, durationFormat(duration))) : null, showMessageListView() ? /*#__PURE__*/React.createElement(View, {
    style: styles.messageListView
  }, /*#__PURE__*/React.createElement(ZegoInRoomMessageView, {
    itemBuilder: messageItemBuilder,
    style: styles.fillParent
  })) : null, isMemberListVisable ? /*#__PURE__*/React.createElement(ZegoLiveStreamingMemberList, {
  ...
  }) : null, /*#__PURE__*/React.createElement(ZegoCoHostMenuDialog, {
  ...
  }), /*#__PURE__*/React.createElement(ZegoToast, {
    visable: isToastVisable,
    text: toastExtendedData.text,
    type: toastExtendedData.type
  }), /*#__PURE__*/React.createElement(ZegoDialog, {
    ...
    style: [styles.dismissArea, {
      bottom: (Platform.OS == 'ios' ? keyboardHeight + textInputHeight : textInputHeight) + 40
    }]
  }), showBottomBar() ? /*#__PURE__*/React.createElement(KeyboardAvoidingView, {
    style: [styles.fillParent, {
      zIndex: 9
    }],
    behavior: 'padding'
  }, Platform.OS != 'ios' && keyboardHeight > 0 ? null : /*#__PURE__*/React.createElement(ZegoBottomBar, {
    menuBarButtonsMaxCount: maxCount,
    menuBarButtons: role === ZegoLiveStreamingRole.host ? hostButtons : role === ZegoLiveStreamingRole.audience ? audienceButtons : role === ZegoLiveStreamingRole.coHost ? coHostButtons : [],
    menuBarExtendedButtons: role === ZegoLiveStreamingRole.host ? hostExtendButtons : role === ZegoLiveStreamingRole.audience ? audienceExtendButtons : role === ZegoLiveStreamingRole.coHost ? coHostExtendButtons : [],
    turnOnCameraWhenJoining: turnOnCameraWhenJoining,
    turnOnMicrophoneWhenJoining: turnOnMicrophoneWhenJoining,
  ...
  }), textInputVisable ? /*#__PURE__*/React.createElement(View, {
    style: [styles.messageInputPannel, {
      bottom: Platform.OS == 'ios' ? keyboardHeight : 0,
      height: textInputHeight
    }]
  }, /*#__PURE__*/React.createElement(ZegoInRoomMessageInput, {
    ref: input => {
      setTextInput(input);
    }
    // @ts-ignore
...
export default /*#__PURE__*/forwardRef(ZegoUIKitPrebuiltLiveStreaming);
